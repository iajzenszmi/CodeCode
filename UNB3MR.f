C
      SUBROUTINE UNB3MR(LATRAD,HEIGHTM,DAYOYEAR,ELEVRAD,
     1 HZD, DHMFDEL, WZD, DWMFDEL, DRATE)
C
...
C     neutral atmospheric delay rate for a given latitude, height, 
...
C        mapping function rates. 
C
C     6. Total slant delay rate is determined using (4) and (5).
C
C   General Comments:
C   -----------------
C
...
C   2006/06/22     Rodrigo Leandro          SUBROUTINE UNB3MR.f created 
...
C     dHMFdel     OUT   DOUBLE     Hydrostatic niell mapping function rate (rad**-1)
C     WZD         OUT   DOUBLE     Non-hyd. zenith delay (m)
C     dWMFdel     OUT   DOUBLE     Non-hyd. Niell mapping function rate (rad**-1)
C     Drate       OUT   DOUBLE     Total slant delay rate (m/rad)
C
C=======================================================================
C
C
...
C
     ...
     &, ES, FW, DSINEDEL, DALPHADEL, DGAMMADEL, DHMFDEL, DHT_CORRDEL
     &, DWMFDEL, DHCCDEL, DRATE
...
C-----------------------------------------------------------------------
      ALPHA  = B/(SINE + C )
      DALPHADEL = (-B/((SINE+C)**2.0D0))*DSINEDEL
      GAMMA  = A/(SINE + ALPHA)
      DGAMMADEL = (-A/((SINE+ALPHA)**2.0D0))*(DSINEDEL+DALPHADEL)
      TOPCON = (1.0D0 + A/(1.0D0 + B/(1.0D0 + C)))
C     HMF    = TOPCON / ( SINE + GAMMA )
      DHMFDEL=(-TOPCON/((SINE+GAMMA)**2.0D0))*(DSINEDEL+DGAMMADEL)
C=======================================================================
C
C
C=======================================================================
...
C-----------------------------------------------------------------------
      ...
      DALPHADEL = (-B_HT/((SINE+C_HT)**2.0D0))*DSINEDEL
      GAMMA  = A_HT/( SINE + ALPHA)
      DGAMMADEL = (-A_HT/((SINE+ALPHA)**2.0D0))*(DSINEDEL+DALPHADEL)
C     HT_CORR_COEF = 1/SINE - HT_TOPCON/(SINE + GAMMA)
      DHCCDEL = ((HT_TOPCON/((SINE+GAMMA)**2.0D0))-
     &1.0D0/(SINE**2.0D0))*DSINEDEL+
     &(HT_TOPCON/((SINE+GAMMA)**2.0D0))*DGAMMADEL
C     HT_CORR = HT_CORR_COEF * HEIGHTM / 1000.0D0
      DHT_CORRDEL = (HEIGHTM / 1000.0D0)*DHCCDEL
C     HMF = HMF + HT_CORR
...
C-----------------------------------------------------------------------
      ...
      DGAMMADEL=(-A/((SINE+ALPHA)**2.0D0))*(DSINEDEL+DALPHADEL)
      TOPCON = (1.0D0 + A/(1.0D0 + B/(1.0D0 + C)))
C     WMF = TOPCON / ( SINE + GAMMA )
      DWMFDEL = (-TOPCON/((SINE+GAMMA)**2.0D0))*(DSINEDEL+DGAMMADEL)
C=======================================================================
C
C
C=======================================================================
C Compute total slant delay rate
C-----------------------------------------------------------------------
